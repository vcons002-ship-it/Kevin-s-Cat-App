...
- A **hit** is a locator-class box scoring in the weak band (``weak_floor`` ≤ score
  ...
  tracks) but they confirm on their own through the normal path.
...
class TrackFuser:
    ...
    def __init__(self, frame_size=None, window: float = WINDOW_SECS,
                 ...
                 reconfirm: float = RECONFIRM_SECS):
        ...
        self._tracks: list[dict] = []         # {"hits": [(ts, score, box)], "cooldown_until": t}
    ...
    def update(self, ts: float | None, hits) -> dict | None:
        ...
        for score, box in sorted(hits or [], key=lambda h: -h[0]):
            ...
            if tr is None:
                ...
                tr = {"hits": [], "cooldown_until": 0.0}
                self._tracks.append(tr)
            tr["hits"].append((now, float(score), box))
        ...
        for tr in self._tracks:
            ...
            if travel < self._min_travel_px():
                continue                       # decoy guard: no movement, no cat
            cand = {
